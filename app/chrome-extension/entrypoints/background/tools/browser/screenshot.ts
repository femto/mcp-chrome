...
class ScreenshotTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: ScreenshotToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      // Check if show last click on screenshot is enabled and draw marker if applicable
      try {
        const debugSettings = await chrome.storage.local.get([
          'showLastClickOnScreenshot',
          'lastClickPosition',
        ]);
        if (debugSettings.showLastClickOnScreenshot && debugSettings.lastClickPosition) {
